...
<?php
// partie validation ajout à la table catdonneeformulaire
if (isset($_POST["valider"])) {
		...
		$objet= new Catdonneeformulaire($libelle,$infobulle);
		$daoCd= new CatdonneeformulaireDAO();
		$rep=$daoCd->create($objet);

	  	$message='Catégorie correctement ajoutée';
        	echo '<script type="text/javascript">window.alert("'.$message.'");</script>';
		echo '<script type="text/javascript">window.location="admin_categorie_donnee.php"</script>';
		Exit();
    	} else {  
...
echo "<table>
	<tr><th class='col-md-4'>Catégories de données</th><th class='col-md-4'>Infobulles</th><th class='col-md-1'>Modification</th><th class='col-md-1'>Suppression</th></tr>";
        	$cat = new CatdonneeformulaireDAO();
        	$readAll = $cat->readAll();
...
<?php 
// partie modification de la table catdonneeformulaire
...
//partie suppression d'un champ de la table catdonneeformulaire
if (isset($_POST["validerSupp"])){   
	$id= $_POST["validerSupp"];
	//supprimer
	$daoCac= new CatdonneeformulaireDAO();
	$daoCac->delete($id);
